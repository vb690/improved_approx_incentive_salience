...
BTCH = np.array([i for i in range(len(os.listdir(f"{INPUTS_PATH}context")))])
...
for tr_vl_i, test_i in SPLITTER.split(BTCH):
    ...
    for btch in TR_VL_BTCH:
        ...
        user_id_tr.append(np.load(f"{TARGETS_PATH}\\user_id\\{btch}.npy"))
    ...
    for model_name in ["RNN", "RNN_env", "RNN_even", "RNN_env_even", "MLP"]:

        optimizer = MODELS[model_name]
        ...
        if model_name == "lag_1":
            ...
            elif model_name == "RNN_even":
                INPUTS = ["continuous_features", "context", "act", "freq"]
            elif model_name in ["RNN_env_even", "MLP"]:
                INPUTS = [
                    "continuous_features",
                    "context",
                    "country",
        ...
        for btch in tqdm(TS_BTCH):
            ...
            btch_context = np.load(f"data\\train\\inputs\\context\\{btch}.npy")
            ...
            if model_name == "lag_1":
                ...
                list_pred = model._model.predict(
                    features, batch_size=array.shape[0]
                )
            elif model_name == "median_model":
        ...
        for context in unique_context:
            ...
            for target_name in TARGETS:
                ...
                error = smape_np(
                    padded_tar[ind, :, :], padded_pred[ind, :, :], axis=-1
                )
