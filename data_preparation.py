...
features = ["delta_sessions", "active_time", "session_time", "activity"]
targets = [
    "user_id",
    "tar_delta_sessions",
    "tar_active_time",
...
embeddings = ["context", "country", "hour", "month", "day_m", "day_w"]
games = ["madness", "lisbf", "lis", "hmg", "jc3", "jc4"]
...
for game in games:
    ...
    df = pd.read_csv(
        f"data\\csv\\{game}.csv", converters={"activity_index_type": eval}
    )
    ...
    # create target
    df["tar_delta_sessions"] = df.groupby("user_id")["delta_sessions"].shift(-1)
