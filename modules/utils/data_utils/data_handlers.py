...
class DataGenerator(Sequence):
    def __init__(self, list_batches, inputs, targets, train=True, shuffle=True):
        """Short summary.

        Args:
            list_batches (type): Description of parameter `list_batches`.
            inputs (type): Description of parameter `inputs`.
            targets (type): Description of parameter `targets`.
            train (type): Description of parameter `train`.
            shuffle (type): Description of parameter `shuffle`.
    ...
    def __getitem__(self, index):
        ...
        Args:
            index (type): Description of parameter `index`.
...
def create_features_batches(
    ...
    Args:
        ...
        sorting_keys (type): Description of parameter `sorting_keys`.
        grouping_key (type): Description of parameter `grouping_key`.
...
def create_embedding_batches(
    ...
    Args:
        df (type): Description of parameter `df`.
        embeddings_keys (type): Description of parameter `embeddings_keys`.
        train (type): Description of parameter `train`.
        id_key (type): Description of parameter `id_key`.
        sorting_keys (type): Description of parameter `sorting_keys`.
        grouping_key (type): Description of parameter `grouping_key`.
    ...
    for key, group in df.groupby(grouping_key):
        ...
        for batch_index in range(num_batches):
            ...
            for embedding, array in arrays.items():
                ...
                np.save(
                    "{}\\inputs\\{}\\{}.npy".format(
                        root_dir, embedding, batch_index_filename
                    ),
                    arr=batch,
                )
                gc.collect()
...
def create_activities_batches(
    ...
    Args:
        df (type): Description of parameter `df`.
        act_columns (type): Description of parameter `act_columns`.
        freq_columns (type): Description of parameter `freq_columns`.
        train (type): Description of parameter `train`.
        id_key (type): Description of parameter `id_key`.
        sorting_keys (type): Description of parameter `sorting_keys`.
        grouping_key (type): Description of parameter `grouping_key`.
    ...
    batch_index_filename = count_files_dir(f"{root_dir}\\inputs\\act")
    for key, group in df.groupby(grouping_key):
        ...
        freq_array = freq_array.reshape((unique_ids, key, len(act_columns)))
...
def create_targets_batches(
    ...
    Args:
        df (type): Description of parameter `df`.
        targets_keys (type): Description of parameter `targets_keys`.
        train (type): Description of parameter `train`.
        id_key (type): Description of parameter `id_key`.
        sorting_keys (type): Description of parameter `sorting_keys`.
        grouping_key (type): Description of parameter `grouping_key`.
    ...
    for key, group in df.groupby(grouping_key):
        ...
        for batch_index in range(num_batches):
            ...
            for target, array in arrays.items():
                ...
                np.save(
                    "{}\\targets\\{}\\{}.npy".format(
                        root_dir, target, batch_index_filename
                    ),
                    arr=batch,
                )
                gc.collect()
...
def csv_handling(
    ...
    Args:
        games_list (type): Description of parameter `games_list`.
        features_keys (type): Description of parameter `features_keys`.
        embeddings_keys (type): Description of parameter `embeddings_keys`.
        activity_key (type): Description of parameter `activity_key`.
        scaler (type): Description of parameter `scaler`.
        id_key (type): Description of parameter `id_key`.
        grouping_key (type): Description of parameter `grouping_key`.
        sorting_keys (type): Description of parameter `sorting_keys`.
        train_size (type): Description of parameter `train_size`.
        n_slices (type): Description of parameter `n_slices`.
    ...
    for game in games_list:
        ...
        df["activity_index_type"] = df["activity_index_type"].apply(
            lambda x: [[] if len(i) == 0 else [np.int32(i[0]), i[1]] for i in x]
        )
        df = df.sort_values(sorting_keys)
    ...
    save_objects(objects=scalers, dir_name="saved_objects\\scalers")
    ...
    unique_values = [value for value in unique_values if type(value) is str]
    mapper = {value: code for code, value in enumerate(unique_values, 1)}
    mappers[activity_key] = mapper

    save_objects(objects=mappers, dir_name="saved_objects\\mappers")
    ...
    for full_df, type_df in zip([df_tr, df_ts], ["train", "test"]):
        ...
        for slice_ind, slice_ids in enumerate(
            ...
            slice_df.to_csv(
                f"data\\{type_df}\\df_{type_df}_{slice_ind}.csv", index=False
            )
...
def array_dumping(
    ...
    """
    with open(
        "results\\saved_objects\\mappers\\activity_index_type.pkl", "rb"
    ) as pickle_file:
    ...
    df[activity_key] = df[activity_key].apply(
        lambda x: [
            []
            if len(i) == 0
            else [np.int32(i[0]), np.int32(activity_mapper[i[1]])]
            for i in x
        ]
    )
