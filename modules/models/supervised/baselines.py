...
class TimeDistributedMLP(_AbstractHyperEstimator):
    ...
    def build(self, hp):
        ...
        freq_input = Input(shape=(None, self.max_activities), name="freq_input")
        ...
        day_month_embedding = self._generate_embedding_block(
            hp=hp, input_tensor=day_month_input, input_dim=32, tag="days_month"
        )
        ...
        features_env = Concatenate(name="features_env_concatenation")(
            [
                area_embedding,
                cont_embedding,
                hour_embedding,
        ...
        features_event = Concatenate(name="features_event_concatenation")(
            [cont_embedding, events_embedding]
        )
        ...
        features_beha = Concatenate(name="features_beha_concatenation")(
            [cont_embedding, feat_input]
        )
        ...
        features = Concatenate(name="features_concatenation")(
            [features_env, features_event, features_beha]
        )
        ...
        absence = TimeDistributed(Dense(units=1), name="output_absence_td")(
            absence
        )
        ...
        active = TimeDistributed(Dense(units=1), name="output_active_td")(
            active
        )
        ...
        sess_time = TimeDistributed(Dense(units=1), name="output_sess_time_td")(
            sess_time
        )
        ...
        activity = TimeDistributed(Dense(units=1), name="output_activity_td")(
            activity
        )
        ...
        sess = self._generate_fully_connected_block(
            hp=hp, input_tensor=dense, tag="sess", prob=self.prob, max_layers=10
        )
        sess = TimeDistributed(Dense(units=1), name="output_sess_td")(sess)
