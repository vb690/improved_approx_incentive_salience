...
for btch in tqdm(BTCH):
    ...
    user_id_array = np.load(
        f'data\\train\\targets\\user_id\\{btch}.npy'
    )
    cont_feat_array = np.load(
        f'data\\train\\inputs\\continuous_features\\{btch}.npy'
    )
    act_array = np.load(
        f'data\\train\\inputs\\act\\{btch}.npy'
    )
    freq_array = np.load(
        f'data\\train\\inputs\\freq\\{btch}.npy'
    )
    cont_array = np.load(
    ...
    for snapshot in range(max_snapshots):
        ...
        users_temporal[snapshot].append(
            user_id_array[:, 0]
        )
        cont_temporal[snapshot].append(
            cont_array[:, 0]
        )
        act_temporal[snapshot].append(
            act_array[:, :snapshot+1, :]
        )
        freq_temporal[snapshot].append(
            freq_array[:, :snapshot+1, :]
        )
    ...
    for emb_name in INPUTS_EMB:
        ...
        emb_array = np.load(
            f'{INPUTS_PATH.format(emb_name)}\\{btch}.npy'
        )
    ...
    for target_index, target_name in enumerate(TARGETS):
        ...
        if target_name != 'tar_sessions':
            ...
            cont_feat_array = cont_feat_array.reshape(
                (-1, cont_feat_shape[2])
            )
            cont_feat_array = SCALER.inverse_transform(
                cont_feat_array
            )
...
with open(
    'results\\saved_objects\\saved_data_containers\\rnn_vs_env_even.pkl',
    'wb'
) as container:
